...
install_ompl()
{
    ...
    if [ -z $GITHUB ]; then
        if [ -z $APP]; then
            wget -O - https://github.com/ompl/${OMPL}/archive/1.5.1.tar.gz | tar zxf -
            cd ${OMPL}-1.5.1
        else
            wget -O - https://github.com/ompl/${OMPL}/releases/download/1.5.1/${OMPL}-1.5.1-Source.tar.gz | tar zxf -
            cd $OMPL-1.5.1-Source
        fi
    else
